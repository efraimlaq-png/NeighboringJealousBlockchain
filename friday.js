...
function buildCommands() {
  const setup = new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        .setName("voice_creator")
        .setDescription("Configura creator voice")
        .addChannelOption((o) =>
          o
            .setName("categoria")
        ...
        .addChannelOption((o) =>
          o
            .setName("canal_texto")
            .setDescription("Canal painel")
            .addChannelTypes(ChannelType.GuildText)
            .setRequired(true)
        )
        .addStringOption((o) => o.setName("nome_mestre").setDescription("Nome da sala mestre"))
    )
    .addSubcommand((s) =>
      s
        .setName("protocolos")
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.isRoleSelectMenu()) {
      ...
      const title = Buffer.from(titleKey, "base64").toString("utf8");
      ...
      const sent = await channel.send({
        embeds: [new EmbedBuilder().setColor(COLORS.CYAN).setTitle(title).setDescription("Auto-cargos")],
        components: [new ActionRowBuilder().addComponents(menu)],
      });
    ...
    if (commandName === "setup") {
      ...
      if (sub === "voice_creator") {
        const category = interaction.options.getChannel("categoria", true);
        const panel = interaction.options.getChannel("canal_texto", true);
        const masterName = interaction.options.getString("nome_mestre") || "Criar Sala";
        ...
        guildConfig.voiceCreator.panelChannelId = panel.id;
        ...
        await publishOrRefreshVoiceAdminPanel(guild, guildConfig, guildState).catch(() => null);
        return safeReply(interaction, {
          content: `Creator Voice configurado. Canal mestre: ${master}`,
          ephemeral: true,
        });
      }
    ...
    if (commandName === "auto_cargos") {
      ...
      const menu = new RoleSelectMenuBuilder()
        .setCustomId(`auto_role_builder:${guild.id}:${interaction.user.id}:${encoded}`)
        .setPlaceholder("Selecione os cargos para o painel")
        .setMinValues(1)
        .setMaxValues(25);
      return safeReply(interaction, {
        content: "Selecione agora os cargos no menu abaixo. Depois publico o painel.",
        components: [new ActionRowBuilder().addComponents(menu)],
        ephemeral: true,
      });
    }
